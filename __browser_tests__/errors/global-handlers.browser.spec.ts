...
describe('setupGlobalErrorHandlers', () => {
	...
	describe('window.onerror handling', () => {
		...
		it('should capture synchronous errors via window.onerror', () => {
			...
			expect(state.currentError?.message).toBe('Sync error test');
			expect(state.currentError?.source).toBe('global');
			expect(state.showErrorUI).toBe(true);
		});
		...
		it('should capture errors with string message only (no Error object)', () => {
			...
			const state = get(errorStore);
			expect(state.currentError).not.toBeNull();
			expect(state.currentError?.message).toBe('String only error');
		});
		...
		it('should include source location in context', () => {
			...
			const state = get(errorStore);
			expect(state.currentError?.context).toEqual({
				source: 'script.js',
				lineno: 42,
				colno: 10
			});
	...
	describe('window.onunhandledrejection handling', () => {
		...
		it('should capture unhandled promise rejections with Error', () => {
			...
			const state = get(errorStore);
			expect(state.currentError).not.toBeNull();
			expect(state.currentError?.message).toBe('Promise rejection test');
			expect(state.currentError?.source).toBe('unhandled-rejection');
		});
		...
		it('should capture unhandled promise rejections with string reason', () => {
			...
			const state = get(errorStore);
			expect(state.currentError).not.toBeNull();
			expect(state.currentError?.message).toBe('String rejection reason');
		});
		...
		it('should capture unhandled promise rejections with object reason', () => {
			...
			expect(state.currentError?.message).toContain('object');
		});
	...
	describe('error severity and UI visibility', () => {
		...
		it('should set severity to error for window.onerror', () => {
			...
			expect(get(errorStore).currentError?.severity).toBe('error');
		});
	...
	describe('multiple error handling', () => {
		...
		it('should capture multiple errors in sequence', () => {
			...
			const state = get(errorStore);
			expect(state.currentError?.message).toBe('Error 3');
			expect(state.errorHistory.length).toBe(3);
		});
		...
		it('should handle mixed error types', () => {
			...
			const state = get(errorStore);
			expect(state.errorHistory.length).toBe(2);
			expect(state.errorHistory[0].source).toBe('unhandled-rejection');
			expect(state.errorHistory[1].source).toBe('global');
		});
	});
});
...
describe('error propagation resilience', () => {
	...
	it('should handle errors even when reporter throws', () => {
		...
		// Error should still be in store
		expect(get(errorStore).currentError?.message).toBe('Test error');
	});
